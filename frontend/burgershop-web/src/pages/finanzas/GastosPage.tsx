...
export default function GastosPage() {
  ...
  // (La búsqueda y el local se excluyen del conteo; aquí no hay otros filtros activos)
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Stats Cards */}
      {stats && (
        <div className={`${statsMobileVisibles ? 'grid' : 'hidden'} sm:grid grid-cols-2 sm:grid-cols-4 gap-4`}>
      ...
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 space-y-3">
        ...
        {/* Filtros completos */}
        <div className={`${filtrosMobileVisibles ? 'flex' : 'hidden'} sm:flex flex-wrap items-end gap-3`}>
          ...
          <button
            onClick={() => { if (mostrarForm && !editandoId) { setMostrarForm(false); } else { abrirForm(); } }}
            className="hidden sm:flex px-2.5 py-1.5 text-[13px] font-medium text-emerald-700 bg-emerald-50 border border-emerald-300 rounded-md hover:bg-emerald-100 items-center gap-1.5"
          >
            <svg className="w-[18px] h-[18px]" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" />
            </svg>
