...
export default function EntregasPage() {
  ...
  const [finalizandoZona, setFinalizandoZona] = useState<number | null>(null);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 pb-2">
            {Array.from(pedidosPorZona.entries()).map(([zonaId, data]) => {
              ...
              return (
                <div
                  key={zonaId}
                  className="bg-white rounded-lg border-2 border-gray-300 shadow-xl flex flex-col max-h-[calc(100vh-16rem)]"
                >
                  {/* Header de zona - sticky */}
                  <div className="sticky top-0 bg-white rounded-t-lg border-b border-gray-200 p-3 flex-shrink-0 z-10 shadow-md">
                    ...
                    {tienePendientes ? (
                      ...
                      <div className="py-1.5">
                        ...
                        {todosFinales ? (
                          <button
                            disabled={finalizandoZona === data.zonaId}
                            onClick={async () => {
                              try {
                                const repIdFin = data.pedidos.find(p => p.repartidorId)?.repartidorId;
                                if (!repIdFin) return;
                                setFinalizandoZona(data.zonaId);
                                ...
                                setFinalizandoZona(null);
                              }
                            }}
                            className="w-full mt-1 py-2 rounded-md font-semibold text-sm text-emerald-700 bg-emerald-50 border border-emerald-300 hover:bg-emerald-100 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
                          >
                            {finalizandoZona === data.zonaId ? (
                              <>
                                <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24">
                                  <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" />
                                  <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z" />
