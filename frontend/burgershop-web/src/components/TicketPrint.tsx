  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 print:bg-transparent print:static print:inset-auto">
      ...
      <div className="relative bg-white rounded-lg shadow-2xl max-h-[90vh] overflow-y-auto print:shadow-none print:rounded-none print:max-h-none print:overflow-visible">
        ...
        <div className="p-4 ticket-content" style={{ width: '80mm', fontSize: '13px', fontFamily: FONT, fontWeight: 600, color: '#000', WebkitFontSmoothing: 'none' }}>
          {/* Encabezado */}
          <div className="text-center mb-3">
            <div style={{ fontSize: '16px', fontWeight: 900 }}>HLP Burger Shop</div>
            <div style={{ fontSize: '11px' }}>Av. Vergara 2485, Hurlingham</div>
            <div style={{ fontSize: '11px' }}>Tel: (011) 4665-1234</div>
          </div>

          <div className="text-center mb-2" style={{ borderTop: '1px dashed #000', borderBottom: '1px dashed #000', padding: '4px 0' }}>
            <div style={{ fontSize: '18px', fontWeight: 900 }}>#{ticket.numeroTicket}</div>
          </div>
          ...
          <table className="w-full mb-2">
            <thead>
              <tr style={{ fontSize: '11px', fontWeight: 700, borderBottom: '1px solid #000' }}>
            ...
            <tbody>
              {ticket.lineas.map((l, i) => (
                <tr key={i} style={{ fontSize: '12px' }}>
                  ...
                  <td className="text-right" style={{ padding: '2px 0', whiteSpace: 'nowrap' }}>
          ...
          <div className="mb-2">
            ...
            <div className="flex justify-between" style={{ fontSize: '16px', fontWeight: 900, borderTop: '2px solid #000', paddingTop: '4px', marginTop: '4px' }}>
              <span>TOTAL:</span>
          ...
          {ticket.pagos && ticket.pagos.length > 0 ? (
            <div className="mb-2">
              <div style={{ fontSize: '11px', fontWeight: 700, marginBottom: '2px' }}>Pagos:</div>
          ...
          {/* Nota interna */}
          {ticket.notaInterna && (
            <div className="mb-2" style={{ fontSize: '11px', fontStyle: 'italic', borderTop: '1px dashed #000', paddingTop: '4px' }}>
              Nota: {ticket.notaInterna}
            </div>
          )}

          <div style={{ borderTop: '1px dashed #000', marginTop: '8px', paddingTop: '8px' }} className="text-center">
            <div style={{ fontSize: '12px' }}>Gracias por su compra!</div>
          </div>
        </div>
      </div>
