...
export default function PedidosDelDiaPanel({
  ...
  const cantidadFiltrosActivos = filtroEstado !== null ? 1 : 0;
  ...
  return (
    <div className="w-full bg-white rounded-lg shadow-2xl border-2 border-slate-300 flex flex-col min-h-0 flex-1">
      <div className="px-3 py-2 border-b-2 border-amber-500 flex-shrink-0 bg-gradient-to-b from-slate-500 to-slate-700 rounded-t-lg shadow-lg">
        ...
        {/* Barra mobile compacta: busqueda + toggle filtros */}
        <div className="sm:hidden flex items-center gap-1.5 mb-1.5">
          ...
          <button
            ...
            className={`px-2 py-1 rounded-md border flex items-center justify-center gap-1 transition-colors ${
              filtrosMobileVisibles || cantidadFiltrosActivos > 0
                ? 'text-amber-700 bg-amber-50 border-amber-300'
                : 'text-slate-200 bg-slate-600 border-slate-500'
            }`}
            aria-label="Filtros"
          >
            ...
            {cantidadFiltrosActivos > 0 && (
              <span className="bg-amber-500 text-white text-[9px] font-bold px-1 py-0 rounded-full">
        ...
        <div className={`${filtrosMobileVisibles ? 'flex' : 'hidden'} sm:flex gap-1 mb-1.5 items-center overflow-x-auto pb-0.5 scrollbar-hide`}>
          <button
