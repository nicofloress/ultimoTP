...
export default function MovimientosPage() {
  ...
  // Filtros activos (excluye local; fechas != hoy cuentan como filtro)
  const hoyStr = getHoy();
  const cantidadFiltrosActivos =
    (fechaDesde !== hoyStr ? 1 : 0) +
    (fechaHasta !== hoyStr ? 1 : 0) +
    (filtroCodigoAccion !== '' ? 1 : 0) +
    (filtroProducto !== '' ? 1 : 0);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="sm:hidden bg-white rounded-lg shadow p-2 flex items-center gap-2">
        ...
        <span className="text-xs text-gray-500 whitespace-nowrap">{movimientosFiltrados.length} mov.</span>
      </div>
